...
const DeviceManagement = () => {
    ...
    // Access the list of devices from the `content` property
    ...
    const [selectedDevice, setSelectedDevice] = useState<DeviceDTO | null>(
        null
    );
    ...
    const filteredDevices = deviceList.filter((device) => {
        const matchesStatus =
            !searchParams.status || device.status === searchParams.status;
        const matchesLocation =
            !searchParams.location || device.location === searchParams.location;
        const matchesSerial =
            !searchParams.serialNumber ||
            device.serialNumber
                .toLowerCase()
                .includes(searchParams.serialNumber.toLowerCase());
        return matchesStatus && matchesLocation && matchesSerial;
    });
    ...
    const handleManageWarranty = async (device: DeviceDTO) => {
        try {
            // Dispatch the getDeviceDetails thunk and unwrap the result
            const result = await dispatch(getDeviceDetails(device.id)).unwrap();
            setSelectedDevice(result); // Set the selected device with the response data
                ...
                <select
                    ...
                    onChange={(e) =>
                        setSearchParams((prev) => ({
                            ...prev,
                            status:
                                (e.target.value as DeviceStatus) || undefined,
                        }))
                    }
                >
                ...
                <select
                    ...
                    onChange={(e) =>
                        setSearchParams((prev) => ({
                            ...prev,
                            location:
                                (e.target.value as DeviceLocation) || undefined,
                        }))
                    }
                >
